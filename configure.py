...
def has_protoc():
    try:
        ...
        return proc.communicate()[0].decode().startswith("libprotoc")
    except OSError:
        return False
...
def can_generate_proto_header():
    try:
        ...
        proc = subprocess.Popen([tool, '--probe'], stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
        return proc.communicate()[0].decode().startswith("ok")
    except OSError:
        return False
...
for name in ['build',
             ...
             'util',
             'version']:
    objs += cxx(name, variables=cxxvariables, order_only=src('frontend.pb.h'))
if platform.is_windows():

...
void StatusSerializer::BuildEdgeStarted(const Edge* edge,
                                        int64_t start_time_millis) {
  ...
  for (vector<Node*>::const_iterator it = edge->outputs_.begin();
...
void StatusSerializer::BuildEdgeFinished(Edge* edge, int64_t end_time_millis,
  ...
  edge_finished->set_status(success ? 0 : 1);
  edge_finished->set_output(output);
